import { PrismaClient, PresetStatus } from '@prisma/client';
...
const sellerSelect = { id: true, username: true, displayName: true, avatar: true } as const;
...
export function createPresetRouter(
  ...
  uploadsDir: string,
  privatePresetsDir: string,
): Router {
  const router = Router();
  const presetMediaDir = path.join(uploadsDir, 'preset-media');
  const stagingDir = path.join(privatePresetsDir, '.staging');
  [presetMediaDir, privatePresetsDir, stagingDir].forEach((dir) => fs.mkdirSync(dir, { recursive: true }));
  ...
  const receiveAssets = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    upload.fields([{ name: 'package', maxCount: 1 }, { name: 'preview', maxCount: 1 }, { name: 'cover', maxCount: 1 }])(
      req,
      res,
      (error: unknown) => {
  ...
  const isOwnerOrAdmin = (preset: { sellerId: string }, user?: AuthenticatedRequest['user']) =>
    Boolean(user && (preset.sellerId === user.id || user.role === 'ADMIN'));
  ...
  router.post('/', authenticateToken, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (!Number.isInteger(price) || price < 0 || price > 10_000_000) return res.status(400).json({ error: 'Invalid price' });
  ...
  router.patch('/:id', authenticateToken, async (req: AuthenticatedRequest, res) => {
    ...
    if (!isOwnerOrAdmin(preset, req.user)) return res.status(403).json({ error: 'Forbidden' });
    const { title, description, priceCents, tags, status } = req.body;
    if (status && !Object.values(PresetStatus).includes(status)) return res.status(400).json({ error: 'Invalid status' });
    if (status === PresetStatus.PUBLISHED && !preset.packageKey) return res.status(400).json({ error: 'Upload a ZIP package before publishing' });
  ...
  router.post('/:id/assets', authenticateToken, receiveAssets, async (req: AuthenticatedRequest, res) => {
    const preset = await prisma.preset.findUnique({ where: { id: req.params.id } });
    if (!preset) return res.status(404).json({ error: 'Preset not found' });
    if (!isOwnerOrAdmin(preset, req.user)) return res.status(403).json({ error: 'Forbidden' });
    ...
    if (!packageFile && !previewFile && !coverFile) return res.status(400).json({ error: 'No files uploaded' });
    const movePublic = (file: Express.Multer.File) => {
      const destination = path.join(presetMediaDir, file.filename);
      fs.renameSync(file.path, destination);
      return `/uploads/preset-media/${file.filename}`;
    };
    const movePrivate = (file: Express.Multer.File) => {
      const destination = path.join(privatePresetsDir, file.filename);
      fs.renameSync(file.path, destination);
    ...
    try {
      ...
      if (packageKey && preset.packageKey) deleteFile(path.join(privatePresetsDir, preset.packageKey));
      const updated = await prisma.preset.update({
        where: { id: preset.id },
        data: {
  ...
  router.post('/:id/checkout', authenticateToken, async (req: AuthenticatedRequest, res) => {
    const preset = await prisma.preset.findUnique({ where: { id: req.params.id } });
    if (!preset || preset.status !== PresetStatus.PUBLISHED || !preset.packageKey) return res.status(404).json({ error: 'Preset not available' });
    if (preset.sellerId === req.user!.id) return res.status(400).json({ error: 'You already own this preset' });
  ...
  router.get('/:id/download', authenticateToken, async (req: AuthenticatedRequest, res) => {
    const preset = await prisma.preset.findUnique({ where: { id: req.params.id } });
    if (!preset?.packageKey || !preset.packageName) return res.status(404).json({ error: 'Package not found' });
    ...
    if (!allowed) return res.status(403).json({ error: 'Purchase required to download this preset' });
    const filePath = path.join(privatePresetsDir, preset.packageKey);
    if (!fs.existsSync(filePath)) return res.status(404).json({ error: 'Package file not found' });
    res.download(filePath, preset.packageName);
  });

  router.patch('/admin/:id/status', authenticateToken, async (req: AuthenticatedRequest, res) => {
    if (req.user!.role !== 'ADMIN') return res.status(403).json({ error: 'Admin access required' });
    const { status } = req.body;
    if (!Object.values(PresetStatus).includes(status)) return res.status(400).json({ error: 'Invalid status' });
  ...
  router.delete('/:id', authenticateToken, async (req: AuthenticatedRequest, res) => {
    ...
    if (!preset) return res.status(404).json({ error: 'Preset not found' });
    if (!isOwnerOrAdmin(preset, req.user)) return res.status(403).json({ error: 'Forbidden' });
    if (preset.packageKey) deleteFile(path.join(privatePresetsDir, preset.packageKey));
  ...
  router.get('/:id', optionalAuth, async (req: AuthenticatedRequest, res) => {
    ...
    if (!preset || (preset.status !== PresetStatus.PUBLISHED && !isOwnerOrAdmin(preset, req.user))) return res.status(404).json({ error: 'Preset not found' });
